...
function translatecodon($codon,$outputtype=1)
	...
	//Make codon upper case to search array
	...
	//Find and return the amino acid
	$aminoacid = $geneticcode[$codon][$outputtype];
	Return $aminoacid;
	}
//---FunctionBreak---
/*Turn degerate codons into a list of natural codons

$codon is the codon

Return is an array containing all codons that the degerate codon encodes.*/
//---DocumentationBreak---
function degeneratecodontocodons($codon)
	{
	//List of different degenerate codons
	$degenerates = array();
	$degenerates['N'] = "ACGT";
	$degenerates['B'] = "CGT";
	$degenerates['D'] = "AGT";
	...
	//Split codon into nucleotides and lists
	$codon = str_split($codon);

	$codon[0] = str_split($degenerates[$codon[0]]);
	$codon[1] = str_split($degenerates[$codon[1]]);
	$codon[2] = str_split($degenerates[$codon[2]]);

	//Count each nucleotide length
	$nucleotide1count = count($codon[0]);
	$nucleotide2count = count($codon[1]);
	$nucleotide3count = count($codon[2]);

	//Output array
	$outputcodons = array();

	//Loop each nucleotide in the codon
	$nucleotide1key = 0;
	while ($nucleotide1key < $nucleotide1count)
		{
		$nucleotide2key = 0;
		while ($nucleotide2key < $nucleotide2count)
			{
			$nucleotide3key = 0;
			while ($nucleotide3key < $nucleotide3count)
				{
				//Generate codon and push to output
				$outputcodon = $codon[0][$nucleotide1key] . $codon[1][$nucleotide2key] . $codon[2][$nucleotide3key];
				array_push($outputcodons,$outputcodon);
				$nucleotide3key++;
				}
			$nucleotide2key++;
			}
